...
data("faithful")
# Basic scatterplot
ggplot(data = faithful, # First, tell it what data to plot
       mapping = aes(x = eruptions, y = waiting)) + # What to use for x and y
  geom_point() # How to draw the plot: geometry point.
# ggplot uses the + sign to add new layers and definitons to the plot. But it's
# pretty similar to the pipe in tidyverse
# Data and mapping can be given both as global (in ggplot()) or per layer
ggplot() + 
  geom_point(mapping = aes(x = eruptions, y = waiting),#here, the data is specific to the layer, which isn't the best because you would have to define the data again each time you maek a new layer 
             data = faithful)
```
...
```{r}
# Mapping color
ggplot(faithful) + #you don't have to specify data=faithful, you can just give it faithful. Lots of people who use ggplot do it this way, even though it makes the code harder to read 
  geom_point(aes(x = eruptions, y = waiting, colour = eruptions < 3)) #this output, where we've mapped color, it automatically gives us a legend that we can use to interpret the plot 
```

If you simple want to set it to a value, put it outside of `aes()`

```{r}
# Setting color. No legend because there is no mapping. This just set every data point to blue.
ggplot(faithful) + 
  geom_point(aes(x = eruptions, y = waiting),
             colour = 'steelblue') 
# ggplot(faithful) + 
  # geom_point(aes(x = eruptions, y = waiting,  colour = 'steelblue'),
            ) #If you put color = into the aes, then fuck you. jk 
# aes is a mapping command. What you're doing here is mapping the expression steelblue to color. So, it's telling R that color should be taken not from the data, but from a fixed variable called steelblue. This creates a legend that says oh we colored every point that has the value steelblue, red.
# so just remember: iF you want to take something from the data, put it inside aes. If you want to set/define something (like you want to make every point 2cm), you set it outside of aes.
```
...
# points are not the only geometries, and not all geometries have the same mapping requirements. For example, you need x and y values to map points. But for histograms, you only need the x value because it will calculate the y by itself. 
# Because the mapping requirements differ, you need to use the help menues to figure out what the geometry requires 
```
...
ggplot(faithful, aes(x = eruptions, y = waiting)) + 
  geom_density_2d() + 
  geom_point()
# plots aren't confined to a single geometry 
# the order of the addition of layers defines the layers of the plot 
```
...
```{r}
ggplot(faithful) + 
  geom_histogram(aes(x = eruptions,fill = waiting < 60), position = 'dodge',alpha=0.25)
```
